	...
	return (
		<div className="border border-gray-200 dark:border-gray-700 rounded-xl p-5 md:p-7 bg-white dark:bg-gray-800">
			...
			<div className="mb-6 block md:flex md:justify-between md:items-start">
      		...
      		{/* 중단 구조 예시 영역 */}
			<div className="bg-gray-50 dark:bg-gray-900 rounded-[10px] mb-6 flex justify-center items-center py-6 px-4 min-h-[160px] md:py-10 md:px-10 md:min-h-[200px]">
				{children}
			</div>
			...
			<div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-5 mb-4 border border-gray-100 dark:border-gray-700">
				<div className="grid grid-cols-1 md:grid-cols-[160px_1fr] gap-3 md:gap-3.5 text-sm">
					<div className="text-gray-600 dark:text-gray-400 font-semibold md:font-medium">트리거 방식</div>
					<div className="text-gray-700 dark:text-gray-300 -mt-2 md:mt-0">{triggerMethod}</div>

					<div className="text-gray-600 dark:text-gray-400 font-semibold md:font-medium">포커스 유지 여부</div>
					<div className="text-gray-700 dark:text-gray-300 -mt-2 md:mt-0">{focusRetention}</div>

					<div className="text-gray-600 dark:text-gray-400 font-semibold md:font-medium">외부 클릭 시 동작</div>
					<div className="text-gray-700 dark:text-gray-300 -mt-2 md:mt-0">{outsideClickBehavior}</div>

					<div className="text-gray-600 dark:text-gray-400 font-semibold md:font-medium">중첩 사용 가능 여부</div>
					<div className="text-gray-700 dark:text-gray-300 -mt-2 md:mt-0">{nestingSupport}</div>
				</div>
			</div>
			...
			{(guideButton || secondaryGuideButton) && (
				<div className="flex gap-2 flex-col md:flex-row md:justify-end">
					{secondaryGuideButton && (
						<button className="h-9 rounded-lg px-4 bg-transparent text-gray-600 dark:text-gray-400 text-sm font-medium border border-gray-300 dark:border-gray-600 cursor-pointer transition-all duration-200 hover:border-gray-400 dark:hover:border-gray-500 hover:text-gray-700 dark:hover:text-gray-300 w-full md:w-auto">
							{secondaryGuideButton.label}
						</button>
					)}
					{guideButton && (
						<button className="h-9 rounded-lg px-4 bg-[#243c84] text-white text-sm font-medium border-0 cursor-pointer transition-all duration-200 hover:opacity-85 w-full md:w-auto">
							{guideButton.label}
						</button>
					)}
				</div>
...
function Popover({ 
	...
	return (
		<div className="relative inline-block">
			...
			{isOpen && (
				<div className={`absolute z-50 ${getPopoverPositionClass()}`}					
					onMouseEnter={() => setIsOpen(true)}
					onMouseLeave={() => setIsOpen(false)}>
			...
			<div className="flex flex-col gap-10">
				...
				<PopoverDesignBlock
					...
					<div className="relative">
						<button className="w-9 h-9 border border-gray-200 dark:border-gray-600 rounded-md bg-white dark:bg-gray-900 flex items-center justify-center cursor-pointer transition-all duration-200 hover:bg-gray-50 dark:hover:bg-gray-700"
						...
						{contextMenuOpen && (
							<div className="absolute top-full right-0 mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg dark:shadow-[0_4px_12px_rgba(0,0,0,0.3)] min-w-[180px] z-50">
								{[
									...
									<button className={`w-full py-2.5 px-3.5 flex items-center gap-2.5 text-sm text-gray-700 dark:text-gray-300 border-0 bg-transparent cursor-pointer text-left transition-all duration-150 hover:bg-gray-50 dark:hover:bg-gray-700 ${
				...
				<PopoverDesignBlock
					...
					<Popover
						position="bottom"
						trigger={
							<Link className="text-sm text-[#243c84] font-medium underline cursor-pointer flex items-center gap-1"
								href="#"								
